  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        setProductsBestSeller(results.data)
        console.log(results)
      } catch (error) {
        ...
        <div className="max-h-[470px] overflow-y-auto rounded-lg bg-white">
          <table className="table-auto w-full">
            <thead className="bg-gray-200 sticky -top-0.5 z-10">
              <tr>
                <th className="px-4 py-2 border text-left bg-green-300">Top 10 productos mas vendidos</th>
                <th className="px-4 py-2 border text-left bg-green-300">Cantidad</th>
              </tr>
            </thead>
            <tbody>
